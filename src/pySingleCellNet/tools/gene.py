...
def what_module_has_gene(
    adata: AnnData,
    target_gene: str,
    mod_slot: str = 'knn_modules'
) -> List[str]:
    """Find which gene modules contain a specific gene.

    Searches through the gene modules stored in adata.uns to find all modules
    that contain the target gene.

    Args:
        adata: AnnData object with gene modules in .uns.
        target_gene: Gene name to search for.
        mod_slot: Key in .uns containing the gene modules dict. Defaults to 'knn_modules'.

    Returns:
        List of module names that contain the target gene.

    Raises:
        ValueError: If mod_slot is not found in adata.uns.

    Example:
        >>> modules = what_module_has_gene(adata, 'Sox2', mod_slot='knn_modules')
    """
    if mod_slot not in adata.uns.keys():
        raise ValueError(f"{mod_slot} has not been identified.")
